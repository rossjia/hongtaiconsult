...
function tierSummary(cells, values, reviews) {
  ...
  for (const c of cells) if (Object.hasOwn(counts2, c.review_tier) && !reviewResolved(values[c.field_id], reviews[c.field_id], c.review_tier)) counts2[c.review_tier]++;
  return { ...counts2, pending: counts2.P1 + counts2.P2 + counts2.P3, highest: ["P1", "P2", "P3"].find((t) => counts2[t]) || "" };
}
...
var Viewer = class {
  constructor() {
    ...
    new ResizeObserver(() => {
      if (this.source && !this.failed && this.zoom === "fit") {
        clearTimeout(this.resizeTimer);
        this.resizeTimer = setTimeout(() => {
          if (!this.failed) this.render(false);
        }, 120);
      }
    }).observe(this.scroll);
  }
  ...
  async render(scrollToAnchor = false) {
    ...
    const key = [source.sha256, pageNumber, this.rotation, this.zoom, this.scroll.clientWidth, devicePixelRatio].join(":");
    ...
    try {
      ...
      const rotation = (page.rotate + this.rotation) % 360, unit = page.getViewport({ scale: 1, rotation }), scale = this.zoom === "fit" ? Math.max(0.25, (this.scroll.clientWidth - 20) / unit.width) : Number(this.zoom), viewport = page.getViewport({ scale, rotation }), dpr = devicePixelRatio || 1, canvas = document.createElement("canvas");
...
function showTierTip(node, c) {
  tierTip.textContent = `P1 ${c.P1} \xB7 P2 ${c.P2} \xB7 P3 ${c.P3}`;
...
function decorateTier(node, c) {
  ...
  node.setAttribute("aria-label", `\u5F85\u6838 ${c.pending}\uFF1B\u6700\u9AD8\u5C42\u7EA7 ${c.highest || "\u65E0"}\uFF1BP1 ${c.P1} \xB7 P2 ${c.P2} \xB7 P3 ${c.P3}`);
...
function getQueue(record, unit) {
  ...
  const q = new SaveQueue({ actor: state.account.id, record: record.record_id, unit, value: v.text, revision: v.revision, request, onState: paintSave, onAck: (q2, res) => {
...
function focusReview(node, follow = true, force = false) {
  ...
  const choices = (node.reviewEvidence || []).flatMap((b) => b.targets.map((t, index) => ({ b, t, index }))), current = viewer.source?.source_id || $("#source-select").value;
...
async function enter(data) {
  ...
  $("#login").hidden = true;
  $("#app").hidden = false;
  $("#identity").textContent = data.account.name;
  notice("");
  await list();
