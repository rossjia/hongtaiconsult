// audits/PC497_REVIEW_CONCISE_ANSWER_RECOVERY_02/RUN_20260923_RECOVERY02/source/config/accounts.json
...
// audits/PC497_REVIEW_CONCISE_ANSWER_RECOVERY_02/RUN_20260923_RECOVERY02/source/src/shared.mjs
...
// audits/PC497_REVIEW_CONCISE_ANSWER_RECOVERY_02/RUN_20260923_RECOVERY02/source/src/review-state.mjs
...
async function reviewSnapshot(db, id) {
  const rows = await db.batch([
    ...
    db.prepare("SELECT c.unit,c.text,c.revision,c.last_author,c.last_modified,p.new_text IS NOT NULL science_patch,p.applied_revision science_patch_revision,p.new_text science_patch_text FROM current_values c LEFT JOIN pc497_answer_baselines p ON p.record_id=c.record_id AND p.field_id=c.unit AND p.applied_revision=c.revision AND p.new_text=c.text WHERE c.record_id=?").bind(id),
...
// audits/PC497_REVIEW_CONCISE_ANSWER_RECOVERY_02/RUN_20260923_RECOVERY02/source/config/field_registry.json
...
// audits/PC497_REVIEW_CONCISE_ANSWER_RECOVERY_02/RUN_20260923_RECOVERY02/source/src/api.mjs
...
var tierJoin = "FROM pc497_risk_tiers t JOIN current_values c ON c.record_id=t.record_id AND c.unit=t.field_id LEFT JOIN field_reviews f ON f.record_id=c.record_id AND f.field_id=c.unit JOIN initial_cells i ON i.record_id=c.record_id AND i.field_id=c.unit LEFT JOIN pc497_answer_baselines p ON p.record_id=c.record_id AND p.field_id=c.unit AND p.applied_revision=c.revision AND p.new_text=c.text";
...
async function recordData(db, id) {
  const results = await db.batch([
    ...
    db.prepare("SELECT field_id,full_text,snapshot_sha,source_run FROM pc497_preconcise_fulltext WHERE record_id=?").bind(id)
  ]);
  ...
  const fulltexts = new Map(results[6].results.map((x) => [x.field_id, x]));
  for (const c of data.cells) {
    const full = fulltexts.get(c.field_id);
    if (full) c.readonly_details = [{ heading: "\u5B8C\u6574\u539F\u7B54\u6848\uFF08\u7B80\u6D01\u5316\u524D\uFF0C\u53EA\u8BFB\uFF09", paragraphs: [full.full_text], button_indices: c.evidence_buttons.map((_, i) => i) }, ...c.readonly_details];
  }
  data.concise_version = "PC497_REVIEW_CONCISE_ANSWER_RECOVERY_02";
...
// audits/PC497_REVIEW_CONCISE_ANSWER_RECOVERY_02/RUN_20260923_RECOVERY02/source/src/function.mjs
